-- Instant power-ups for online mode. Replaces the collect-to-inventory / arm-and-click
-- model: landing on a hidden tile now applies the power-up's effect immediately and the
-- client plays a slow reveal (showing what it is) before the effect lands.
--
-- IMPORTANT: this MIRRORS the offline rules in localGame.js (stepRoll / applyTilePower /
-- applyMystery) and gameLogic.js (resolveMoveWithPowerUps). Keep them in sync: power-up
-- ids, tile counts (6 power / 3 mystery) and the MYSTERY outcome order
-- ['advance','retreat','grant','extra'].
--
-- Instant mapping on landing (must match applyTilePower):
--   shield              -> standing shield (blocks the NEXT snake, then clears)
--   doubleRoll/extraRoll-> an extra turn (a double can't retro-apply post-move)
--   swap                -> swap with the current leader now
--   freeze              -> freeze the current leader's next turn
-- power_state now carries { tiles, shield, frozen } (no more `inventory`).

-- roll_dice no longer takes an armed power-up; drop both old signatures.
drop function if exists public.roll_dice(uuid, text);
drop function if exists public.roll_dice(uuid);

-- ── create_room: seed power_state without an inventory ───────────────────────
...
-- ── roll_dice: instant power-up resolution (mirrors localGame.stepRoll) ───────
...
begin
  ...
  -- Current leader among the other active roles (for swap / freeze), pre-move.
  ...
  -- Resolve the move (overshoot bounce, snake/ladder, a standing shield negates a snake).
  v_raw := v_pos + v_roll;
  if v_raw > 100 then
    v_new := v_pos;
  else
  ...
  if v_power_ups and v_raw <= 100 and v_new <> 100 then
    ...
    if v_tile = 'power' then
      ...
      if v_mkind = 'advance' then
        ...
        v_events := v_events || jsonb_build_object('kind', 'mystery', 'role', caller_role, 'at', v_landsq, 'outcome', 'advance', 'to', v_new);
      elsif v_mkind = 'retreat' then
        v_new := greatest(0, v_new - 4);
        v_events := v_events || jsonb_build_object('kind', 'mystery', 'role', caller_role, 'at', v_landsq, 'outcome', 'retreat', 'to', v_new);
      elsif v_mkind = 'extra' then
        v_extra := true;
        v_events := v_events || jsonb_build_object('kind', 'mystery', 'role', caller_role, 'at', v_landsq, 'outcome', 'extra', 'to', v_new);
      else -- grant: apply a random instant power (revealed to the client as that power)
        v_grant := pool[floor(random() * 5)::int + 1];
      end if;
    end if;

    -- Apply the instant power (from a power tile, or a mystery 'grant').
    if v_grant = 'shield' then
      v_shield := true;
      v_events := v_events || jsonb_build_object('kind', 'acquire', 'role', caller_role, 'id', 'shield', 'at', v_landsq);
    elsif v_grant in ('doubleRoll', 'extraRoll') then
      v_extra := true;
      v_events := v_events || jsonb_build_object('kind', 'acquire', 'role', caller_role, 'id', v_grant, 'at', v_landsq);
    elsif v_grant = 'freeze' then
      if v_leader is not null then
        v_state := jsonb_set(v_state, array['frozen', v_leader], 'true'::jsonb);
      end if;
      v_events := v_events || jsonb_build_object('kind', 'acquire', 'role', caller_role, 'id', 'freeze', 'at', v_landsq, 'target', v_leader);
    elsif v_grant = 'swap' then
      if v_leader is not null then v_swap := true; end if;
      v_events := v_events || jsonb_build_object('kind', 'acquire', 'role', caller_role, 'id', 'swap', 'at', v_landsq, 'with', v_leader);
    end if;
  end if;

  -- Resolve final positions (a swap trades the caller's tile square for the leader's spot).
  v_caller_final := v_new;
  v_leader_final := null;
  if v_swap and v_leader is not null then
    v_leader_final := v_new;        -- leader takes the caller's landed tile
    v_caller_final := v_leadpos;    -- caller takes the leader's pre-swap spot
  end if;
  ...
  -- Turn advancement: winner / extra-turn keep the turn; else next active role,
  ...
  -- Persist the caller's standing shield (path only exists with power-ups on).
  if v_power_ups then
...
-- ── rematch: reset power-up state (regenerate tiles, clear shield/frozen) ─────
...
-- `_pu_remove_one` (inventory splice helper) is no longer used.
drop function if exists public._pu_remove_one(jsonb, text);
